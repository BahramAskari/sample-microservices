...
import LoginSession, {LoginSession__Output} from "./LoginSession";
import {Role__Output} from "./Role";
...
export interface User__Output extends Required<User__Attributes> {
    ...
    LoginSessions?: LoginSession__Output[]
    Roles?: Role__Output[]
}
...
class User extends Model<User__Attributes, User__Input> implements User__Attributes {
    ...
    //  Many -> One
    declare getLoginSessions: HasManyGetAssociationsMixin<LoginSession>; // Note the null assertions!
    declare countLoginSessions: HasManyCountAssociationsMixin; // Note the null assertions!
    declare createLoginSession: HasManyCreateAssociationMixin<LoginSession>;       // Token check not required
    declare addLoginSession: HasManyAddAssociationMixin<LoginSession, LoginSession["id"]>;       // Token check not required
    declare addLoginSessions: HasManyAddAssociationsMixin<LoginSession, LoginSession["id"]>;     // Token check not required
    declare removeLoginSession: HasManyRemoveAssociationMixin<LoginSession, LoginSession["id"]>; // Token check not required
    declare removeLoginSessions: HasManyRemoveAssociationsMixin<LoginSession, LoginSession["id"]>;   // Token check not required
    declare setLoginSessions: HasManySetAssociationsMixin<LoginSession, LoginSession["id"]>; // Token check not required
    ...
    declare static associations: {
        ...
        LoginSessions: Association<User, LoginSession>;
    };
